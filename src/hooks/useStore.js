...
import { getEmbeddingsBatch, formatErrorForUser } from '../lib/api.js';
import { sortGroupChildren, findNode, isDescendant, removeNode, insertInto, sortOnlyAffectedNode, volumeCacheRef } from '../lib/tree.js';
import { mergeGroupsWithLLM } from '../lib/group-merger.js';
...
const useStore = () => {
  ...
  const autoGroup = async () => {
    ...
    try {
      ...
      const userMessage = formatErrorForUser ? formatErrorForUser(err) : (err?.message || String(err));
  ...
  const refineGroups = async () => {
    ...
    try {
      // NOTA: window.refineGroupsWithLLM aún no ha sido migrado a ES module
  ...
  const mergeSimilarGroups = async (threshold = 0.6) => {
    ...
    try {
      ...
      const result = await mergeGroupsWithLLM(
        tree,
        threshold,
        (message) => {
          setSuccess(message);
