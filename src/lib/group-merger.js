 ...
 */
export const computeGroupEmbedding = (group) => {
 ...
 */
export const buildSimilarityMatrix = (embeddings) => {
 ...
 */
export const findGroupCliques = (groups, embeddings, threshold = 0.6) => {
 ...
 */
export const mergeGroupsWithLLM = async (tree, threshold = 0.6, progressCallback = null) => {
  console.log('\n════════════════════════════════════════════════════');
  ...
  try {
    ...
    const serverBase2 = (window.location.protocol === 'file:' ||
                       window.location.hostname === 'localhost' ||
                       window.location.hostname === '127.0.0.1')
      ? 'http://localhost:3000'
      : '';

    const response = await fetch(`${serverBase2}/api/merge-groups`, {
      method: 'POST',
