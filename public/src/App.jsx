...
const { ReactFlow, ReactFlowProvider, Controls, MiniMap, Background, useNodesState, useEdgesState, MarkerType } = window.ReactFlow || {};
  ...
  const Node = ({node, depth=0, parentCollapsed=false})=>{
    ...
    return (
      <div key={node.id} className="mb-3 animate-fade-in" style={{marginLeft: depth? '32px':'0'}}>
        <div
          draggable
          onDragStart={(e)=>{ setDragging(node); e.dataTransfer.effectAllowed='move'; }}
          ...
          onDrop={(e)=>{ e.preventDefault(); setDragOver(null); if (dragging) onDrop(node, dragging); }}
            ...
            {isEditing ? (
              <div className="flex items-center gap-2 flex-1">
                <input
                  ...
                  autoFocus
                  onKeyDown={(e)=>{
                    if(e.key==='Enter'){ renameNode(node.id, editingText); setEditingId(null); }
                    if(e.key==='Escape'){ setEditingId(null); }
                  }}
                />
                <button onClick={()=>{ renameNode(node.id, editingText); setEditingId(null); }}
              ...
              <>
                ...
                {isGroup && (
                  <button onClick={()=>{ setEditingId(node.id); setEditingText(node.name||''); }}
